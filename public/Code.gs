 ...
 * ============================================
 * Google Sheet에 붙어서 동작하는 서버 로직입니다.
 *
 * 관리자 인증
 *  - 스크립트 소유자 계정으로 접속하면 이메일로 자동 인증됩니다.
 *  - 다른 일반 Gmail 계정은 구글이 이메일을 넘겨주지 않으므로,
 *    설정 시트의 `관리자키` 값을 URL에 붙여 접속합니다.
 *      .../exec?page=admin&key=발급한키
 */
...
var HEAD_찬양주보 = ['날짜', '토요연습시간', '이주의말씀', '말씀설명', '유튜브재생목록', '방송팀요청'];
var WW_날짜 = 0, WW_연습 = 1, WW_말씀 = 2, WW_설명 = 3, WW_재생목록 = 4, WW_방송 = 5;
...
var HEAD_찬양행사 = ['ID', '이름', '종류', '날짜', '끝날짜', '연습날짜', '연습시간', '장소', '메모', '만든이', '등록시각'];
var WE_ID = 0, WE_이름 = 1, WE_종류 = 2, WE_날짜 = 3, WE_끝 = 4, WE_연습날짜 = 5, WE_연습시간 = 6,
    WE_장소 = 7, WE_메모 = 8, WE_만든이 = 9, WE_시각 = 10;
...
/* =========================================================
   ...
   (시트에서 직접 고친 경우도 onEdit 이 판을 바꿉니다)
   ========================================================= */
var _캐시판 = null;
...
/** 시트를 직접 고쳐도 캐시가 새로 읽히도록 (간단 트리거 — 따로 설정할 것 없습니다) */
function onEdit(e) { 캐시판갈기_(); }
...
function isAdmin_(key) {
  var 관리키 = 설정값_('관리자키');
  if (관리키 && String(key || '').trim() === 관리키) return true;

  var me = (Session.getActiveUser().getEmail() || '').toLowerCase().trim();
  if (!me) return false;
  var list = 설정값_('관리자이메일').toLowerCase().split(',');
...
function whoami_() { return Session.getActiveUser().getEmail() || '(알 수 없음)'; }
...
function 대리안내메일_(info, cell, until, by) {
  ...
  try {
    var url = (ScriptApp.getService().getUrl() || '') + '?page=portal';
    MailApp.sendEmail({
      to: info.email,
...
/** 설정 확인용 — 스크립트 편집기에서 직접 실행하세요 */
function 설정확인() {
  var base = ScriptApp.getService().getUrl() || '(배포 후 확인 가능)';
  SpreadsheetApp.getUi().alert(
    '★ 모두에게 나눠줄 링크 (포털) ★\n' + base + '?page=portal\n' +
    '  이름 + 전화번호 10자리로 들어가면 각자 역할에 맞는 메뉴가 열립니다.\n\n' +
    '내 계정: [' + Session.getActiveUser().getEmail() + ']\n' +
    '관리자이메일: [' + 설정값_('관리자이메일') + ']\n' +
    '관리자키: [' + 설정값_('관리자키') + ']\n' +
    '시간대: ' + Session.getScriptTimeZone() + '\n\n' +
    '관리 페이지 링크:\n' +
    (ScriptApp.getService().getUrl() || '(배포 후 확인 가능)') +
    '?page=admin&key=' + 설정값_('관리자키') + '\n\n' +
    '새가족팀 링크:\n' +
    (ScriptApp.getService().getUrl() || '(배포 후 확인 가능)') + '?page=newfamily\n\n' +
    '사역팀 링크:\n' +
    (ScriptApp.getService().getUrl() || '(배포 후 확인 가능)') + '?page=team\n\n' +
    '지출환급신청서 링크 (누구나 접근 가능):\n' +
    (ScriptApp.getService().getUrl() || '(배포 후 확인 가능)') + '?page=expense');
}
...
function render_(file, title, params, iconKey) {
  ...
  t.prefill = JSON.stringify(params).replace(/</g, '\\u003c');
  var out = t.evaluate().setTitle(title)
    .addMetaTag('viewport', 'width=device-width, initial-scale=1');

  // 설정 시트에 아이콘 주소가 있으면 탭 아이콘으로 사용합니다
  var url = 설정값_('아이콘:' + iconKey);
  if (url) { try { out.setFaviconUrl(url); } catch (e) {} }
  return out;
}
...
function 배정대상알림_() {
  ...
  var url = (ScriptApp.getService().getUrl() || '') + '?page=newfamily';
  MailApp.sendEmail({
    to: to,
  ...
  return {
    ...
    worship: (function () {
      ...
      return {
        ...
        url: ScriptApp.getService().getUrl() + '?page=worship&key=' + encodeURIComponent(설정값_('관리자키') || '')
      };
    })(),
    expense: (function () {
      var list = 지출목록_();
...
function notifyTeamReport_(team, data, isEdit) {
  ...
  var url = (ScriptApp.getService().getUrl() || '') + '?page=team';
  var opts = {
    to: to,
...
function getReminderState(key) {
  ...
  return {
    ...
    sundayScheduled: triggers.filter(function (t) { return t.getHandlerFunction() === '주일독려'; }).length > 0,
    mondayScheduled: triggers.filter(function (t) { return t.getHandlerFunction() === '미제출리마인더'; }).length > 0,
    weekOf: ymd_(이번주기준_())
  };
}
...
function 리마인더트리거설치() {
  ScriptApp.getProjectTriggers().forEach(function (t) {
    if (t.getHandlerFunction() === '미제출리마인더') ScriptApp.deleteTrigger(t);
  });
  ScriptApp.newTrigger('미제출리마인더').timeBased()
    .onWeekDay(ScriptApp.WeekDay.MONDAY).atHour(8).create();

  var tz = Session.getScriptTimeZone();
  var 경고 = (tz === 'America/Toronto' || tz === 'America/New_York') ? ''
    : '\n\n\u26a0 현재 시간대가 ' + tz + ' 입니다.\nProject Settings에서 Eastern Time - Toronto로 변경해주세요.';
  SpreadsheetApp.getUi().alert(
    '리마인더가 켜졌습니다.\n매주 월요일 오전 8시에 미제출 셀장에게 발송됩니다.' + 경고);
}

function 주일독려트리거설치() {
  ScriptApp.getProjectTriggers().forEach(function (t) {
    if (t.getHandlerFunction() === '주일독려') ScriptApp.deleteTrigger(t);
  });
  ScriptApp.newTrigger('주일독려').timeBased()
    .onWeekDay(ScriptApp.WeekDay.SUNDAY).atHour(16).nearMinute(30).create();

  var tz = Session.getScriptTimeZone();
  var 경고 = (tz === 'America/Toronto' || tz === 'America/New_York') ? ''
    : '\n\n\u26a0 현재 시간대가 ' + tz + ' 입니다.\nProject Settings에서 Eastern Time - Toronto로 변경해주세요.';
  SpreadsheetApp.getUi().alert(
    '주일 제출 안내가 켜졌습니다.\n매주 주일 오후 4시 30분경, 아직 제출하지 않은 셀장에게 발송됩니다.' + 경고);
}

function 리마인더미리보기() {
  var me = Session.getActiveUser().getEmail();
  sendReminderMail_({ leader: '홍길동', name: '홍길동 셀', email: me },
    ymd_(이번주기준_()), ScriptApp.getService().getUrl() || 'https://script.google.com/');
  SpreadsheetApp.getUi().alert(me + ' 로 월요일 리마인더 미리보기 메일을 보냈습니다.');
}

/** 지출 접수 메일이 어떻게 보이는지 내 계정으로 한 번 보내봅니다 */
function 지출신청미리보기() {
  var me = Session.getActiveUser().getEmail();
  지출접수메일_({
  ...
  SpreadsheetApp.getUi().alert(me + ' 로 지출 접수 확인 메일 미리보기를 보냈습니다.');
}
...
function 주일독려미리보기() {
  ...
  sendSundayNudge_({ leader: '홍길동', name: '홍길동 셀', email: me },
    ymd_(이번주기준_()), ScriptApp.getService().getUrl() || 'https://script.google.com/');
  SpreadsheetApp.getUi().alert(me + ' 로 주일 제출 안내 미리보기 메일을 보냈습니다.');
}
...
function 주보하나_(date) {
  var 기본 = { practice: 시간문자열_(설정값_('찬양연습시간')) || '10:30', verse: '', verseNote: '', playlist: '', media: '' };
...
function 한주_(date) {
  date = 예배키_(date);
  var ev = 행사키냐_(date) ? 행사찾기_(date) : null;
  if (행사키냐_(date) && !ev) throw new Error('행사를 찾지 못했습니다. 지워졌을 수 있습니다.');
...
function 예배키_(raw) {
  var k = 날짜문자열_(raw);
  if (/^\d{4}-\d{2}-\d{2}$/.test(k) || 행사키냐_(k)) return k;
...
function saveWorshipEvent(token, ev) {
  ...
  }, [row], [{ col: WE_날짜 }, { col: WE_끝 }, { col: WE_연습날짜 }, { col: WE_연습시간, fix: 시간문자열_ }]);
...
function setMyUnavailableMany(token, dates, reason) {
  ...
  (dates || []).forEach(function (d) {
    d = 날짜문자열_(d);
    if (/^\d{4}-\d{2}-\d{2}$/.test(d) || 행사키냐_(d)) want[d] = 1;
  });
...
function 포털주소_() { return ScriptApp.getService().getUrl() + '?page=portal'; }
...
function 포털관리메뉴_(r) {
  if (r.roles.indexOf('커미티') === -1) return [];
  var base = (ScriptApp.getService().getUrl() || '') + '?page=admin&key=' + encodeURIComponent(설정값_('관리자키') || '');
  return [
...
function 헌금신청메일_(me) {
  ...
  var url = (ScriptApp.getService().getUrl() || '') + '?page=admin&scope=acct';
...
function getExpenses(key) {
  ...
  return {
    ...
    settings: {
      ...
      contact: 설정값_('회계담당자'),
      formUrl: (ScriptApp.getService().getUrl() || '') + '?page=expense'
    },
    today: ymd_(new Date()),
    counts: counts,
    stats: {
...
function 교적생년월일정리__원래() {
  ...
  SpreadsheetApp.getUi().alert(fixed + '개 행의 생년월일 형식을 정리했습니다.');
}
...
function 새가족시트정리() {
  ...
  var res = ui.alert('새가족 시트 구조 변경',
    '새가족 관련 4개 시트를 이름 기준으로 바꿉니다.\n\n' +
    '먼저 스프레드시트 사본을 만들어 두셨나요?\n계속하시겠습니까?', ui.ButtonSet.YES_NO);
...
function onOpen() {
  SpreadsheetApp.getUi().createMenu('셀보고 관리')
    .addItem('최초 설정 (시트 생성)', '최초설정')
    .addItem('설정·링크 확인', '설정확인')
    .addItem('지출 신청서 미리보기 메일', '지출신청미리보기')
    .addItem('교적 생년월일 정리', '교적생년월일정리')
    .addItem('새가족 시트 구조 정리 (1회)', '새가족시트정리')
    .addItem('주일 제출 안내 켜기 (오후 4:30)', '주일독려트리거설치')
    .addItem('월요일 리마인더 켜기 (오전 8시)', '리마인더트리거설치')
    .addItem('주일 제출 안내 미리보기', '주일독려미리보기')
    .addItem('월요일 리마인더 미리보기', '리마인더미리보기')
    .addToUi();
}
