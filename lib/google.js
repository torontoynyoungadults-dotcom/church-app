...
const store = {
   ...
   * 너무 짧게 잡으면 구글이 알려주는 수정 시각이 우리 쓰기 직후에도 흔들려서
   * 매 요청마다 모든 시트를 다시 읽게 됩니다 — 그래서 5초 아래로는 내려가지 않습니다.
   */
  checkExternal() {
    let sec = Number(process.env.SHEET_CHECK_SECONDS);
    if (!isFinite(sec) || sec <= 0) sec = 10;
    sec = Math.min(Math.max(sec, 5), 600);
    const every = sec * 1000;
    ...
    if (this.knownModified && m && m !== this.knownModified) {
      // 우리가 막 쓴 뒤에는 구글 수정 시각이 몇 초 더 흔들립니다 —
      // 그때마다 전부 다시 읽으면 느려지므로 최소 간격을 둡니다
      if (Date.now() - (this.lastReset || 0) >= 4000) {
...
function makeServices(exec) {
  ...
  const MailApp = {
    sendEmail(a, b, c, d) {
      ...
      if (!process.env.GOOGLE_REFRESH_TOKEN && !(process.env.GMAIL_USER && process.env.GMAIL_APP_PASSWORD)) {
        console.warn('[메일 건너뜀] 메일 설정(GOOGLE_REFRESH_TOKEN 또는 GMAIL_APP_PASSWORD)이 없습니다 →', m.to, m.subject);
        return;
      }
